...
class EmoneyNamesRepository {
  ///
  IsarCollection<EmoneyName> getCollection({required Isar isar}) => isar.emoneyNames;

  ///
  Future<EmoneyName?> getEmoneyName({required Isar isar, required int id}) async {
    final emoneyNamesCollection = getCollection(isar: isar);
  ...
  ///
  Future<void> inputEmoneyName({required Isar isar, required EmoneyName emoneyName}) async {
    final emoneyNamesCollection = getCollection(isar: isar);
    await isar.writeTxn(() async => emoneyNamesCollection.put(emoneyName));
  }

  ///
  Future<void> updateEmoneyName({required Isar isar, required EmoneyName emoneyName}) async {
    final emoneyNamesCollection = getCollection(isar: isar);
