...
class BankPricesRepository {
  ...
  ///
  Future<List<BankPrice>?> getSelectedBankPriceList({required Isar isar, required Map<String, dynamic> param}) async {
    final bankPricesCollection = getCollection(isar: isar);
  ...
  ///
  Future<void> inputBankPriceList({required Isar isar, required List<BankPrice> bankPriceList}) async {
    bankPriceList.forEach((element) => inputBankPrice(isar: isar, bankPrice: element));
  }

  ///
  Future<void> inputBankPrice({required Isar isar, required BankPrice bankPrice}) async {
    final bankPricesCollection = getCollection(isar: isar);
    await isar.writeTxn(() async => bankPricesCollection.put(bankPrice));
  }

  ///
  Future<void> deleteBankPriceList({required Isar isar, required List<BankPrice> bankPriceList}) async {
    bankPriceList.forEach((element) => deleteBankPrice(isar: isar, id: element.id));
  }
